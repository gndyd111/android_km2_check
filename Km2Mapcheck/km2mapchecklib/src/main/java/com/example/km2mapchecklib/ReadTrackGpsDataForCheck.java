...
public class ReadTrackGpsDataForCheck  extends Thread{
    ...
    private ReadTrackGpsDataForCheck()
    {
    }

    private final static ReadTrackGpsDataForCheck instance = new ReadTrackGpsDataForCheck();

    public static ReadTrackGpsDataForCheck getInstance(){
        return instance;
    }
    ...
    private Boolean SearchPack(int buffLen) {
        ...
        int ksxtIndex = gpsString.indexOf("$KSXT");

        if(gpggaIndex < 0 || gprmcIndex < 0 || pashrIndex < 0 || ksxtIndex < 0)
            return false;

        int xhIndexOfKsxt = -1;
        int endIndexOfGpgga = gpsString.indexOf("\r\n", gpggaIndex);
        String jyfOfKsxt = null;
        if(endIndexOfGpgga >= 0) {
            int endIndexOfGprmc = gpsString.indexOf("\r\n", endIndexOfGpgga);
            if (endIndexOfGprmc >= 0) {
                int endIndexOfPashr = gpsString.indexOf("\r\n",endIndexOfGprmc);
                if(endIndexOfPashr >= 0) {
                    int startIndexOfKsxt = gpsString.indexOf("$KSXT",endIndexOfPashr);
                    if(startIndexOfKsxt > 0) {
                        xhIndexOfKsxt = gpsString.indexOf("*", startIndexOfKsxt);
                        if(xhIndexOfKsxt > 0)
                        {
                            //处理剩余数据
                            mBuffer = new byte[buffersize + 1];
                            String leftString = gpsString.substring(xhIndexOfKsxt + 1);
                            int nLeftLen = buffLen - (xhIndexOfKsxt + 1);//剩余有效字符个数
                            if(nLeftLen > 0)
                            {
        ...
        String subGpsString = gpsString.substring(gpggaIndex, xhIndexOfKsxt + 1);
        String gpggaStr = null, gprmcStr = null, pashrStr = null, ksxtStr = null;
        ...
        for(int i = 0; i < arrGpsString.length; i++)
        {
            ...
            if(arrGpsString[i].contains("$KSXT")) {
                if(ksxtStr == null)
                    ksxtStr = arrGpsString[i];
            }
        }
        ProcessPack(gpggaStr, gprmcStr, pashrStr, ksxtStr);
        if(gpggaStr == null || gprmcStr == null || pashrStr == null || ksxtStr == null)
            return false;

        return true;
    }

    private void ProcessPack(String gpggaStr, String gprmcStr, String pashrStr, String ksxtStr)
    {
        try {
            if (Verify(gpggaStr) && Verify(gprmcStr) /*&& Verify(pashrStr)*/) {
                ...
                if (locationData.quality != 0) {
                    locationData.extendStr = ksxtStr;
